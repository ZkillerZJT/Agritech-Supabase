...
#define ONE_WIRE_BUS 4 //se esta utilizando puerto GPIO4 para el onewire
...
int humSensorID = 32;
int humSensor2ID = 35;
const int wet35 = 2625;  //valor de sequedad para sensor en puerto 35
const int dry35 = 930;   //limite de humedad max para sensor en puerto 35
const int wet32 = 2610;
const int dry32 = 912;
...
void httpSetup(String jsonData, const String link) {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        const unsigned long timeoutDuration = 6000; // Timeout of 6 seconds
        ...
        if ((millis() - startTime) >= timeoutDuration) {
            Serial.println("Timeout after 6 seconds!");
...
void loop() {
  ...
  tempSerialPrint();
  showHumedad(humSensor,wet32,dry32); // esta wea no está imprimiendo nada
  int humedad = convertirHum(wet32,humSensor);
  //showHumedad(humSensor2,wet35,dry35);
  //String jsonPayload = sensorPayloadJson(humSensor);  // Solo incluye humedad
  float tempC = sensors.getTempC(insideThermometer);
  Serial.print(String(sensorPayloadJson(humedad, tempC, lightSensor)));//agregar la temperatura, en un valor previamente asociado
  if ((tempC != 85) && (humSensor > dry32) && (humSensor < wet32)){ // hasta aqui todo ok, sensor humedad malo parece.
    httpSetup(String(sensorPayloadJson(humedad, tempC, lightSensor)), endpoint);
  }
